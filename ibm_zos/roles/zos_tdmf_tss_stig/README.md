# IBM z/OS TDMF for TSS STIG (`zos_tdmf_tss_stig`)

Assessment + command-reference **skeleton** for the DISA **IBM z/OS TDMF for TSS STIG**.
...
| `artifacts_dir` | `"/tmp/zos-tdmf-tss-stig-artifacts"` | No | — |
| `zos_live_assessment` | `false` | No | — |
| `zos_target` | `""` | No | — |
| `zos_tdmf_catalog` | `(see defaults/main.yml)` | No | Confirm exact V-IDs/rule-IDs against the TDMF (TSS) STIG release at audit. |
| `compliance_frameworks` | `(see defaults/main.yml)` | No | — |
| `stig_benchmark` | `"IBM z/OS TDMF for TSS STIG"` | No | — |
...
```yaml
- name: Use zos_tdmf_tss_stig
  hosts: all
  gather_facts: false
  roles:
    - role: zos_tdmf_tss_stig
```
...
```bash
cd ibm_zos/roles/zos_tdmf_tss_stig/playbooks

# Checklist only (no mainframe) — command reference + remediation
ansible-playbook -i inventory.example run.yml
cat /tmp/zos-tdmf-tss-stig-artifacts/zos_tdmf_tss_stig_checklist.md
