# IBM z/OS CICS Transaction Server (TSS) STIG (`zos_cics_tss_stig`)

Assessment + command-reference **skeleton** for the DISA **IBM z/OS CICS Transaction Server (TSS) STIG**.
...
| `artifacts_dir` | `"/tmp/zos-cics-tss-stig-artifacts"` | No | — |
| `zos_live_assessment` | `false` | No | — |
| `zos_target` | `""` | No | — |
| `zos_cics_catalog` | `(see defaults/main.yml)` | No | CICS SIT (System Initialization Table) parameters are verified in the region startup JCL/SIT or via CEMT/CESN; TSS-protected resources via TSS LIST(RDT). Confirm exact V-IDs/rule-IDs against the CICS TS (TSS) STIG release at audit. |
| `compliance_frameworks` | `(see defaults/main.yml)` | No | — |
| `stig_benchmark` | `"IBM z/OS CICS Transaction Server (TSS) STIG"` | No | — |
| `stig_version` | `"V7R2"` | No | — |
| `zos_esm` | `TSS` | No | — |

## Example Playbook

```yaml
- name: Use zos_cics_tss_stig
  hosts: all
  gather_facts: false
  roles:
    - role: zos_cics_tss_stig
```
...
```bash
cd ibm_zos/roles/zos_cics_tss_stig/playbooks

# Checklist only (no mainframe) — command reference + remediation
ansible-playbook -i inventory.example run.yml
cat /tmp/zos-cics-tss-stig-artifacts/zos_cics_tss_stig_checklist.md
