# IBM zSecure Suite STIG (`zos_zsecure_stig`)

Assessment + command-reference **skeleton** for the DISA **IBM zSecure Suite STIG**.
...
| `artifacts_dir` | `"/tmp/zos-zsecure-stig-artifacts"` | No | — |
| `zos_live_assessment` | `false` | No | — |
| `zos_target` | `""` | No | — |
| `zos_zsecure_catalog` | `(see defaults/main.yml)` | No | — |
| `compliance_frameworks` | `(see defaults/main.yml)` | No | — |
| `stig_benchmark` | `"IBM zSecure Suite STIG"` | No | — |
...
```yaml
- name: Use zos_zsecure_stig
  hosts: all
  gather_facts: false
  roles:
    - role: zos_zsecure_stig
```
...
```bash
cd ibm_zos/roles/zos_zsecure_stig/playbooks

# Checklist only (no mainframe) — command reference + remediation
ansible-playbook -i inventory.example run.yml
cat /tmp/zos-zsecure-stig-artifacts/zos_zsecure_stig_checklist.md
