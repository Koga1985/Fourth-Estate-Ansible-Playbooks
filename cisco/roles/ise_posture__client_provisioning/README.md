...
| Variable | Default | Description |
|---|---|---|
| `client_provisioning_profiles` | (required) | List of native supplicant profile definitions. Each entry has: `name` (required), optional `description`, and `wireless_profiles` (list of wireless profile configurations with SSID, security, and certificate details) |

### Feature Flags

| Variable | Default | Description |
|---|---|---|
| `ise_posture__client_provisioning_enabled` | `true` | Master toggle for this role |
...
| Variable | Default | Description |
|---|---|---|
| `ise_posture__client_provisioning_log_level` | `INFO` | Log verbosity level |
| `ise_posture__client_provisioning_log_to_syslog` | `true` | Forward events to syslog |
| `ise_posture__client_provisioning_syslog_server` | `{{ vault_syslog_server }}` | Syslog server address |
| `ise_posture__client_provisioning_notify_on_completion` | `false` | Send email on completion |
| `ise_posture__client_provisioning_notification_email` | `{{ vault_security_team_email }}` | Notification recipient |
| `ise_posture__client_provisioning_auto_backup` | `true` | Trigger ISE backup after changes |
