...
| Variable | Default | Required | Description |
|---|---|---|
| `tacacs_command_sets` | (required) | No | List of command set definitions. Each entry has: `name` (required), `commands` (list of command ACEs), `permit_unmatched` (default `false`), and optional `description` |

### TACACS Shell Profiles

| Variable | Default | Required | Description |
|---|---|---|
| `tacacs_profiles` | (required) | No | List of shell profile definitions. Each entry has: `name` (required), `privilege_level` (default `15`), and optional `description` |

### Feature Flags

| Variable | Default | Required | Description |
|---|---|---|
| `ise_policy__shell_profiles_cmdsets_enabled` | `true` | No | Master toggle for this role |
...
| Variable | Default | Required | Description |
|---|---|---|
| `ise_policy__shell_profiles_cmdsets_log_level` | `INFO` | No | Log verbosity level |
| `ise_policy__shell_profiles_cmdsets_log_to_syslog` | `true` | No | Forward events to syslog |
| `ise_policy__shell_profiles_cmdsets_syslog_server` | `{{ vault_syslog_server }}` | **Yes** | Syslog server address |
| `ise_policy__shell_profiles_cmdsets_notify_on_completion` | `false` | No | Send email on completion |
| `ise_policy__shell_profiles_cmdsets_notification_email` | `{{ vault_security_team_email }}` | **Yes** | Notification recipient |
| `ise_policy__shell_profiles_cmdsets_auto_backup` | `true` | No | Trigger ISE backup after changes |
