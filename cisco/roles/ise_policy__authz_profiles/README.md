...
| Variable | Default | Required | Description |
|---|---|---|
| `authz_profiles` | (required) | No | List of authorization profile definitions. Each entry supports: `name` (required), `access_type` (default `ACCESS_ACCEPT`), `vlan` (optional), `dacl` (optional dACL name), and `sgt` (optional Security Group Tag name) |

### Feature Flags

| Variable | Default | Required | Description |
|---|---|---|
| `ise_policy__authz_profiles_enabled` | `true` | No | Master toggle for this role |
...
| Variable | Default | Required | Description |
|---|---|---|
| `ise_policy__authz_profiles_log_level` | `INFO` | No | Log verbosity level |
| `ise_policy__authz_profiles_log_to_syslog` | `true` | No | Forward events to syslog |
| `ise_policy__authz_profiles_syslog_server` | `{{ vault_syslog_server }}` | **Yes** | Syslog server address |
| `ise_policy__authz_profiles_notify_on_completion` | `false` | No | Send email on completion |
| `ise_policy__authz_profiles_notification_email` | `{{ vault_security_team_email }}` | **Yes** | Notification recipient |
| `ise_policy__authz_profiles_auto_backup` | `true` | No | Trigger ISE backup after changes |
