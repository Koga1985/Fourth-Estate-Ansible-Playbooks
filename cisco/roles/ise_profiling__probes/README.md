...
| Variable | Default | Description |
|---|---|---|
| `profiling_dhcp_probe_enabled` | (required) | Enable DHCP probe (ISE listens for DHCP traffic) |
| `profiling_dhcp_span_enabled` | (required) | Enable DHCP SPAN probe (DHCP traffic mirrored to ISE via SPAN) |
| `profiling_radius_probe_enabled` | (required) | Enable RADIUS probe (extract profiling data from RADIUS packets) |
| `profiling_snmp_probe_enabled` | (required) | Enable SNMP Query probe (ISE polls network devices via SNMP) |
| `profiling_snmp_trap_enabled` | (required) | Enable SNMP Trap probe (network devices send traps to ISE) |
| `profiling_netflow_probe_enabled` | (required) | Enable NetFlow probe (ISE receives NetFlow records) |
| `profiling_dns_probe_enabled` | (required) | Enable DNS probe (reverse DNS lookups for endpoint hostnames) |

### Feature Flags

| Variable | Default | Description |
|---|---|---|
| `ise_profiling__probes_enabled` | `true` | Master toggle for this role |
...
| Variable | Default | Description |
|---|---|---|
| `ise_profiling__probes_log_level` | `INFO` | Log verbosity level |
| `ise_profiling__probes_log_to_syslog` | `true` | Forward events to syslog |
| `ise_profiling__probes_syslog_server` | `{{ vault_syslog_server }}` | Syslog server address |
| `ise_profiling__probes_notify_on_completion` | `false` | Send email on completion |
| `ise_profiling__probes_notification_email` | `{{ vault_security_team_email }}` | Notification recipient |
| `ise_profiling__probes_auto_backup` | `true` | Trigger ISE backup after changes |
