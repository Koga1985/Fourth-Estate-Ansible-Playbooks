...
| Variable | Default | Description |
|---|---|---|
| `byod_portal_name` | (required) | Name of the BYOD portal to create or update |
| `byod_allowed_interfaces` | (required) | ISE interface(s) on which the portal listens |
| `byod_cert_group` | (required) | Certificate group tag used for portal TLS |
| `byod_endpoint_group` | (required) | Endpoint identity group for BYOD devices |

### Certificate Provisioning

| Variable | Default | Description |
|---|---|---|
| `byod_cert_profile_name` | (required) | Name of the certificate provisioning profile |
| `byod_cert_authority` | (required) | ISE internal CA to issue BYOD certificates |

### Authorization Profiles

| Variable | Default | Description |
|---|---|---|
| `byod_authz_profiles` | `[]` | List of authorization profiles; each entry has `name`, `access_type`, and optional `vlan` |

### Feature Flags

| Variable | Default | Description |
|---|---|---|
| `ise_byod__workflow_enabled` | `true` | Master toggle for this role |
| `enable_disa_stig_compliance` | `true` | Apply STIG-compliant configuration settings |

### Logging and Notifications

| Variable | Default | Description |
|---|---|---|
| `ise_byod__workflow_log_level` | `INFO` | Log verbosity level |
| `ise_byod__workflow_log_to_syslog` | `true` | Forward events to syslog |
| `ise_byod__workflow_syslog_server` | `{{ vault_syslog_server }}` | Syslog server address |
| `ise_byod__workflow_notify_on_completion` | `false` | Send email on completion |
| `ise_byod__workflow_notification_email` | `{{ vault_security_team_email }}` | Notification recipient |
| `ise_byod__workflow_auto_backup` | `true` | Trigger ISE backup after changes |
